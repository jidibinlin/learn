...
writer.writerow(('职位', '公司简称', '薪资', '公司地址', '工作经验', '学历', '行业',
                '融资状况', '公司人数', '联系人', '发布时间', '职位描述+任职资格', '链接'))
...
def parse_one_page(html):
    ...
    for item in divs.items():
        ...
        job_url = 'https://www.zhipin.com' + \
            item('.info-primary a').attr('href')

        # 职位描述+任职资格
        detail = get_detail_info(job_url)

        writer.writerow((title, company_short_Name, salary, addr, workEx, edu,
                        detail, profess, finace, num_person, contacts, pub_time, job_url))
        print(title, company_short_Name, salary, addr, workEx, edu,
              profess, finace, num_person, contacts, pub_time, detail, job_url)
...
def main():
    urls = ['https://www.zhipin.com/c100010000-p100101/?query=Hadoop&page={0}&ka=page-{1}'.format(
