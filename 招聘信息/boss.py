...
class BossCrawler:
    ...
    def get_city(self):
        ...
        r = requests.get(
            "http://www.zhipin.com/wapi/zpCommon/data/city.json", headers=headers)
        data = r.json()
        ...
        for page_num in range(1, 11):
            ...
            for line in lines:
                ...
                if self.query.lower() not in job.lower():
                    continue
                salary = info_primary.find('span', class_="red").text.split(
                    '-')[0].replace('K', '')
                company = line.find(
                    'div', class_="info-company").find('a').text.lower()
                result = dict(
                    zip(self.csv_header, [city, job, salary, company]))
                print(result)
    ...
    def read_csv(self):
        ...
        data.groupby('city').mean()['salary'].to_frame(
